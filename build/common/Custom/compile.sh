...
[[ ! $firmware == "openwrt_amlogic" ]] && {
	...
	case $RELE in
		...
		*)
			...
			Github="https://github.com/mdtycl/Compile-OpenWrt"
		;;
	esac
}
[[ "${REG_UPDATE}" == "true" ]] && {
	[[ -z ${Git} ]] && Git="https://github.com/mdtycl/Compile-OpenWrt"
...
svn co https://github.com/mdtycl/Compile-OpenWrt/trunk/build $Home/build > /dev/null 2>&1
[[ $? -ne 0 ]] && {
	TIME r "编译脚本下载失败，请检测网络或更换节点再尝试!"
	exit 1
}
svn co https://github.com/mdtycl/Compile-OpenWrt/trunk/common $Home/build/common
[[ $? -ne 0 ]] && {
